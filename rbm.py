...
class RBM(object):
    def __init__(self, n_input, n_hidden, layer_names,alpha=1.0,keep_prob=1.0, transfer_function=tf.nn.sigmoid):
        ...
        self.h0prob = tf.nn.dropout(self.h0prob, self.keep_prob)
        self.h0 = self.sample_prob(self.h0prob)
        self.h0 = tf.nn.dropout(self.h0, self.keep_prob)
        ...
        self.v1 = tf.nn.dropout(self.v1, self.keep_prob)
        ...
        self.h1prob = tf.nn.sigmoid(tf.matmul(self.v1, self.rbm_w) + self.rbm_hb)
        self.h1prob = tf.nn.dropout(self.h1prob, self.keep_prob)
        self.h1 = self.sample_prob(self.h1prob)
        self.h1 = tf.nn.dropout(self.h1prob, self.keep_prob)
    ...
    def reconstruct(self,batch):
        return self.sess.run(self.v1,feed_dict={self.x:batch,
                                                self.rbm_w: self.o_w,
    ...
    def compute_cost(self, batch):
        ...
        return self.sess.run(self.err_sum, feed_dict={self.x: batch, self.rbm_w: self.o_w,
                                                      self.rbm_vb: self.o_vb, self.rbm_hb: self.o_hb,self.keep_prob:1})
    ...
    def partial_fit(self, batch_x):
        ...
        self.n_w, self.n_vb, self.n_hb = self.sess.run([self.update_w, self.update_vb, self.update_hb],
                                                       feed_dict={self.x: batch_x, self.rbm_w: self.o_w,
                                                                  self.rbm_vb: self.o_vb, self.rbm_hb: self.o_hb,self.keep_prob:self.keep_prob_value})
        ...
        return self.sess.run(self.err_sum, feed_dict={self.x: batch_x, self.rbm_w: self.n_w, self.rbm_vb: self.n_vb,
                                                      self.rbm_hb: self.n_hb,self.keep_prob:1})
